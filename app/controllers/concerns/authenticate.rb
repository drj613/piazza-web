module Authenticate
  ...
  included do
    ...
    # declared as a helper method so it can be used in views
    helper_method :logged_in?
  end

  class_methods do
    # completely bypass auth (like for the login page)
    def skip_authentication(**options)
      skip_before_action :authenticate, options
      skip_before_action :require_login, options
    end

    # pages that need to know if you're logged in, but are still visible
    #  to logged out users
